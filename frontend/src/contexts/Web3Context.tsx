...
const initOnboard = (subscriptions: Subscriptions): API => {
  const network = process.env.NODE_ENV === 'development' ? { networkId: 1 } : { networkId: 1 }; // mainnet
...
const Web3Provider: React.FC = ({ children }: Props) => {
  ...
  const updateWallet = useCallback((newWallet: Wallet) => {
    ...
    const network = process.env.NODE_ENV === 'development' ? { name: 'mainnet', chainId: 1 } : undefined;
